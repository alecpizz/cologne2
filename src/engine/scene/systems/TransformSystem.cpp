...
namespace cologne
{
    void TransformSystem::on_update(float dt)
    {
        ...
        {
            update_children(entity, registry);
        }
    }

    void TransformSystem::update_children(entt::entity parent, entt::registry &registry)
    {
        ...
        {
            Entity child = _scene->get_entity_by_uuid(child_id);
            if (registry.valid(child))
            {
                ...
                {
                    update_children(child, registry);
                }
            }
        }
    }
