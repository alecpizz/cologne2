...
namespace cologne
{
    static int32_t translate_internal_format_to_format(uint32_t format)
    {
        switch (format)
        {
            ...
            default:
                LOG_ERROR("GLInternalFormatToGLFormat: Unsupported internal format");
                return 0;
        }
    }
                                        ...
                                        int32_t mag_filter)
    {
        ...
        color_attachment.format = translate_internal_format_to_format(internal_format);
        color_attachment.type = translate_internal_format_to_type(internal_format);
        ...
        glNamedFramebufferTexture(_handle, GL_COLOR_ATTACHMENT0 + static_cast<GLenum>(_color_attachments.size()) - 1,
                                              ...
                                              int32_t wrap, glm::vec4 border_color)
    {
        ...
        glTextureParameterfv(_depth_attachment.handle, GL_TEXTURE_BORDER_COLOR, glm::value_ptr(border_color));
    ...
    void FrameBuffer::draw_buffer(const char *attachment_name)
    {
        ...
        {
            if (strcmp(attachment_name, _color_attachments[i].name) == 0)
            {
    ...
    void FrameBuffer::clear_attachment(const char *attachment_name, float_t r, float_t g, float_t b, float_t a)
    {
        for (size_t i = 0; i < _color_attachments.size(); i++)
        {
            if (strcmp(attachment_name, _color_attachments[i].name) == 0)
            {
                uint32_t texture = _color_attachments[i].handle;
                uint32_t format = _color_attachments[i].format;
                uint32_t type = _color_attachments[i].type;
    ...
    void FrameBuffer::resize(uint32_t width, uint32_t height)
    {
        ...
        for (size_t i = 0; i < _color_attachments.size(); i++)
        {
            ColorAttachment &color_attachment = _color_attachments[i];
    ...
    uint32_t FrameBuffer::get_color_attachment_slot_by_name(const char *name) const
    {
        for (size_t i = 0; i < _color_attachments.size(); i++)
        {
            if (strcmp(name, _color_attachments[i].name) == 0)
            {
                return GL_COLOR_ATTACHMENT0 + static_cast<uint32_t>(i);
            }
        }
