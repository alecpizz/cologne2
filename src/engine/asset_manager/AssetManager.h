...
namespace cologne::AssetManager
{
    ...
    {
        if constexpr (std::is_same_v<Asset, Mesh>)
        {
            return get_mesh_by_name(name);
        }
        if constexpr (std::is_same_v<Asset, SkinnedMesh>)
        {
            return get_skinned_mesh_by_name(name);
        }
        if constexpr (std::is_same_v<Asset, Texture>)
        {
            return get_texture_by_name(name);
        }
        if constexpr (std::is_same_v<Asset, Model>)
        {
            return get_model_by_name(name);
        }
        if constexpr (std::is_same_v<Asset, SkinnedModel>)
        {
            return get_skinned_model_by_name(name);
        }
